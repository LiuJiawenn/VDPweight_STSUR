...
def data_generator(patch_per_frame=64, patch_rate=0.4, key_frame_nb=12, shuffle = True):
    # TODO：
    # 1. 按keyframe读取帧转换为patch
    # 2. 生成optical 转换为patch
    # 3. 读取VDP
    # 4. 准备SURlabel
    raw_path = 'D:/workspace/matlab/JND_prediction/dataset/264videos/'
    running_folder = 'C:/Users/Jiawen/Desktop/runningFolder/'
    vdp_path = 'D:/workspace/pycharm/VDPweight_STSUR/data/training/VDP/'
    kf_list = np.load('data/kf12_list.npy')
    SUR_GROUND_TRUTH = np.load("data/SUR_GROUND_TRUTH.npy")
    trainVideos = np.array(list(range(44, 220)))
    ...
    for i in trainVideos:
        ...
        s_avi = running_folder + current_video + tag + '_qp_00.avi'

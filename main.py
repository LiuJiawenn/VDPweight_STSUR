...
def training(model, patch_rate, epoch_nb=30):
    ...
    try:
        for epoch in range(epoch_nb):
            ...
            np.save('loss_history/0116batch_loss_epoch'+str(epoch)+'.npy', batch_mse_history)
            torch.save(model.state_dict(), 'weights/0116model_weights_epoch'+str(epoch)+'.pth')

    except Exception as err:
        print(f"训练过程中发生了错误：{err}", "     当前已训练视频：", count)
        traceback.print_exc()
    finally:
        np.save('loss_history/batch_loss_final.npy', batch_mse_history)
